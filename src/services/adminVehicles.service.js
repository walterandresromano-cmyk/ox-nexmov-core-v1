...
export async function updateAdminVehicleStatus({ vehicleId, action }) {
  if (!isSupabaseConfigured || !supabase) {
    return {
      vehicle: null,
      error: {
  ...
  const { data, error } = await supabase.rpc("update_admin_vehicle_status", {
...
export async function updateAdminVehicleData(form) {
  if (!isSupabaseConfigured || !supabase) {
    return {
      vehicle: null,
      error: {
  ...
  const { data, error } = await supabase.rpc("update_admin_vehicle_data", {
    ...
    p_force_approve: Boolean(form.forceApprove),
  });
